...
agent-radio/
  ...
  config.yaml                 # Runtime config (gitignored; generated by install.sh)
  scripts/
  ...
  deploy/
    docker/                   # Dockerfiles, docker-compose.yml, config.yaml.example
    k8s/                      # Kubernetes example manifests
    systemd/                  # systemd service unit templates
  audio/
    tones/                    # Short WAV/OGG sound effects for agent events
    starter-music/            # CC0 seed tracks (shipped with repo)
...
See `scripts/start.sh` for the full implementation. High-level flow:

```bash
# 1. Adopt or start Icecast via systemd
# 2. Start Liquidsoap, wait for Unix socket (up to 15s)
# 3. Start brain, wait for port bind (up to 30s)
# 4. Monitor: log warnings if brain exits, keep music running
# 5. On SIGINT/SIGTERM: delegate to scripts/stop.sh and exit
```

Key design decisions:
- `RADIO_DIR` defaults to `/opt/agent-radio` (install location, not repo path)
- Brain crash does NOT kill Liquidsoap — music never stops
- Uses PID files under `$RADIO_DIR` for process tracking
